 ...
 * 
 * InventoryMenu.java
 * The inventory of the game which extends BorderPane and displays an interactive inventory menu containing all items the player receives. It can be created as an instance so it can be added to the scene in WebsiteTemplate
 */
...
public class InventoryMenu extends BorderPane { // Class for displaying inventory as an interactive menu
	...
	public InventoryMenu() { // Menu construction
		...
		descriptionColumn.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getKey().getDescription()));

		TableColumn<Map.Entry<Item, Integer>, Integer> quantityColumn = new TableColumn<>("Quantity");
		quantityColumn
				.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getValue()).asObject());
		...
		// listener to detect which item is highlighted, prompting user based on item
		// information
		...
		statDisplay = new Label("ATK: " + Utility.getPlayerATK() + "(+" + Utility.getTempATK() + ")   DEF: "
				+ Utility.getPlayerDEF() + "(+" + Utility.getTempDEF() + ")");
